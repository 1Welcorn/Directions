...
import { CardData, DifficultyLevel } from '../types';
import { speakText, generateImage } from '../services/gemini';
              ...
              <div className={`mt-2 sm:mt-5 flex items-center justify-between w-full gap-2 border-t border-slate-200/50 pt-2 sm:pt-4`}>
                ...
                {(card.isFlipped || card.isMatched) && (
                  <div className="flex gap-1 sm:gap-2">
                    <button 
                      onClick={handleSpeak} 
                      className={`${isFullScreen ? 'w-6 h-6 sm:w-10 sm:h-10' : 'w-8 h-8 sm:w-12 sm:h-12'} rounded-lg sm:rounded-xl bg-slate-50 text-indigo-600 shadow-sm flex items-center justify-center hover:bg-indigo-600 hover:text-white transition-all active:scale-90 border border-slate-200`}
                    >
                    ...
                    {!card.imageUrl && (
                      <button 
                        ...
                        disabled={genLoading} 
                        className={`${isFullScreen ? 'w-6 h-6 sm:w-10 sm:h-10' : 'w-8 h-8 sm:w-12 sm:h-12'} rounded-lg sm:rounded-xl bg-slate-50 text-amber-500 shadow-sm flex items-center justify-center hover:bg-amber-500 hover:text-white transition-all active:scale-90 border border-slate-200 ${genLoading ? 'animate-pulse' : ''}`}
                      >
